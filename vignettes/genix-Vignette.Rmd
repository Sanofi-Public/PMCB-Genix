...
vignette: >
  %\VignetteIndexEntry{Comparative analysis of gene association networks using 
...
![Sargent logo.](genix-logo.png){width=75% height=75%}
...
comparative model to uncover condition-relevant genes. This vignette gives an 
...
A small example database, `genixDemoData`, is included in the `genix`
package. This database contains the gene expression matrix of 200 cells (`gex`). 
The expression matrix is from a single-cell RNA-seq dataset and is formatted
with genes as rows and cells as columns. This database also contains an
associated meta-data (`metadata`). We extract the relevant information for this
demo using the following commands:
...
There are multiple columns in `metadata`. Typical `seurat` metadata columns including 
`orig.ident`, `nCount_RNA`, and `nFeature_RNA`. In addition, it includes 
`group` column which identifies to which group each cell belongs to:
