...
`cmpl_rslts` objects to `plotNetDegree` function: 
...
The function first calculates the similarity between `grph.1` and `grph.2` using the
...
`grph.1` and `grph.2` are reconstructed by randomly sampling from
...
differ from the observed jaccard index between `grph.1` and `grph.2`.
...
`grph.1` and `grph.2` inferred networks. First `compareNets` function performs an 
...
the normalized degree of connectivity for each gene within the `grph.1` and `grph.2`, 
...
Positive values indicate that the feature is larger in the grph.2. All this information 
can be retrived by: 
...
# fetch top topological variant feasture
